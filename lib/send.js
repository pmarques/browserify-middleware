...
function compile(path, options, cb, cacheUpdated) {
  ...
  if( options.appendBefore ) {
    if( !Array.isArray(options.appendBefore) ) {
		console.error( 'appendBefore options must be an Array!' );
    }

	var _ap = options.appendBefore;
	for( var i =0 ; i < _ap.length; ++i ) {
		console.log( _ap[i] );
	  apb += fs.readFileSync( _ap[i] );
	}

	console.log( apb.length );
  }
  cb = guard(cb);
